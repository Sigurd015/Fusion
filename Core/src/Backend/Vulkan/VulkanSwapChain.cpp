...
namespace Fusion
{
	...
	void VulkanSwapChain::InitSurface(void* handle)
	{
		...
		FUSION_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		FUSION_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t* width, uint32_t* height, bool vsync)
	{
		...
		FUSION_ASSERT(presentModeCount > 0);
