...
namespace Fusion
{
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		FUSION_ASSERT(gpuCount > 0, "No device with Vulkan support found");
		// Enumerate devices
		...
		{
			FUSION_ERROR("Renderer - Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		FUSION_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		FUSION_ASSERT(queueFamilyCount > 0, "");
		...
		if (extCount > 0)
		{
			...
			{
				FUSION_INFO("Renderer - Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					FUSION_INFO("Renderer  {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		FUSION_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		FUSION_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
		...
		FUSION_ASSERT(result == VK_SUCCESS);
